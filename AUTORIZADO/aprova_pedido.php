<?php 
	...
	$atualiza_pedido = "UPDATE pedido SET autorizador = $matricula, fk_Estado = 5 WHERE cod_pedido = $idPedido";
	$result = mysql_query($atualiza_pedido,$con);
    if($result)
    {
        echo"<script type='text/javascript'>alert('Pedido enviado com sucesso ao Almoxarifado!');window.location.href='acompanhar_Pedido_Autorizador.php';</script>";
    }else {
        echo"<script type='text/javascript'>alert('Erro ao autorizar o pedido!');window.location.href='acompanhar_Pedido_Autorizador.php';</script>";
    }
